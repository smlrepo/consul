...
func initialize() {
	effectiveMaxBlocks = adjustMaxBlocks()
...
func checkFreedPortsOnce() {
	...
	if retained > 0 {
		logf("WARN", "%d out of %d pending ports are still in use; something probably didn't wait around for the port to be closed!", pending-len(remove), pending)
	}
...
// port range.
func adjustMaxBlocks() int {
	ephemeralPortMin, ephemeralPortMax, err := getEphemeralPortRange()
	if err != nil {
		logf("WARN", "ephemeral port range detection skipped due to error: %v", err)
		return maxBlocks
	}
...
func Take(n int) (ports []int) {
	...
	if n > total-1 {
		panic(fmt.Sprintf("freeport: block size too small"))
	}

	for len(ports) < n {
		for freePorts.Len() == 0 {
			if total == 0 {
				panic("impossible; there are no actual free ports in the block anymore")
			}
			condNotEmpty.Wait()
		}
...
// represented by [min1, max1] and [min2, max2] overlap.  This function panics
// if the intervals are not well formed (i.e. if min > max).
func intervalOverlap(min1, max1, min2, max2 int) bool {
	if min1 > max1 {
		panic(fmt.Sprintf("interval1 is not ordered [%d, %d]", min1, max1))
	}
	if min2 > max2 {
		panic(fmt.Sprintf("interval2 is not ordered [%d, %d]", min2, max2))
	}
...
func Get(n int) (ports []int) { return Take(n) }

// Deprecated: Please use Take/Return calls instead.
func GetT(t testing.T, n int) (ports []int) { return Take(n) }

// Deprecated: Please use Take/Return calls instead.
func Free(n int) (ports []int, err error) { return Take(n), nil }
